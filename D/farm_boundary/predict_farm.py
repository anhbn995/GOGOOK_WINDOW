        ...
        with tqdm(total=len(list_coordinates)) as pbar:
            with rasterio.open(path_predict,'w+', **meta, compress='lzw') as r:
                for x_off, y_off, x_count, y_count, start_x, start_y in list_coordinates:
                    values = raster.read(window=Window(x_off, y_off, x_count, y_count))[0:3]
...
if __name__=="__main__":
    img_path = r'./test_2.tif'
    out_path = r'./model/test_2.geojson'
    model_path = r"./model_farm.h5"
    ...
    predict_farm(model_farm, args.input_path, cache_path, size)
