...
from greencover.unet_models.libs.activations_base import GELU, Snake
from greencover.unet_models.libs._backbone_zoo import backbone_zoo, bach_norm_checker
from tensorflow.keras.layers import ReLU, LeakyReLU, PReLU, ELU, Softmax, Conv2D, Input
from greencover.unet_models.libs.layer_base import encode_layer, decode_layer, CONV_stack, CONV_output


def UNET_downstack(X, channel, kernel_size=3, stack_num=2, activation='ReLU', 
              pool=True, batch_norm=False, name='down_0'):
    pool_size = 2
...
def UNET_upstack(X, X_list, channel, kernel_size=3, stack_num=2, activation='ReLU',
                 unpool=True, batch_norm=False, concat=True, conv_after=False, name='right0'):
    pool_size = 2
...
def unet_base(input_tensor, filter_num, stack_num_down=2, stack_num_up=2, 
    ...
    if backbone is None:
        ...
        for i, f in enumerate(filter_num[1:]):
            X = UNET_downstack(X, f, stack_num=stack_num_down, activation=activation, pool=pool, 
    ...
    # backbone cases
        ...
        if depth_encode < depth_:
            ...
            for i in range(depth_-depth_encode):
                ...
                X = UNET_downstack(X, filter_num[i_real], stack_num=stack_num_down, activation=activation, pool=pool, 
    ...
    for i in range(depth_decode):
        X = UNET_upstack(X, [X_decode[i],], filter_num_decode[i], stack_num=stack_num_up, activation=activation, 
    ...
    if depth_decode < depth_-1:
        for i in range(depth_-depth_decode-1):
            ...
            X = UNET_upstack(X, None, filter_num_decode[i_real], stack_num=stack_num_up, activation=activation, 
                       unpool=unpool, batch_norm=batch_norm, concat=False, name='{}_up{}'.format(name, i_real))   
    return X

def unet(input_size, filter_num, n_labels, stack_num_down=2, stack_num_up=2,
    ...
    # base    
    X = unet_base(IN, filter_num, stack_num_down=stack_num_down, stack_num_up=stack_num_up, 
...
#     OUT = Conv2D(n_labels, kernel_size=1, padding='same', use_bias=True, name='{}_output'.format(name))(X)
...
def main():
    ...
    n_labels = 3
    model = unet(input_size, filter_num, n_labels, stack_num_down=2, stack_num_up=2,
                activation='ReLU', output_activation='Softmax', batch_norm=False, pool=True, unpool=True, 
                backbone=None, weights='imagenet', freeze_backbone=True, freeze_batch_norm=True, name='unet')
