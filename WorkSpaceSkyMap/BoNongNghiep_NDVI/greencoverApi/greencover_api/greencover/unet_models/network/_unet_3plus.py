...
from greencover.unet_models.network._unet import UNET_downstack, UNET_upstack
from tensorflow.keras.layers import Conv2D, Activation, concatenate, Softmax
...
def unet_3plus_base(input_tensor, filter_num_down, filter_num_skip, filter_num_aggregate, 
    ...
    * Upsampling is achieved through bilinear interpolation and can be replaced by transpose convolutional layers here.
    ...
    if backbone is None:
        ...
        for i, f in enumerate(filter_num_down[1:]):
            ...
            X = UNET_downstack(X, f, kernel_size=3, stack_num=stack_num_down, activation=activation, 
        ...
        if depth_encode < depth_:
            ...
            for i in range(depth_-depth_encode):
                ...
                X = UNET_downstack(X, filter_num_down[i_real], stack_num=stack_num_down, activation=activation, pool=pool, 
    ...
    if depth_decode < depth_-1:
        for i in range(depth_-depth_decode-1):
            ...
            X = UNET_upstack(X, None, filter_num_aggregate, stack_num=stack_num_up, activation=activation, 
...
def unet_3plus(input_size, n_labels, filter_num, filter_num_skip='auto', filter_num_aggregate='auto', 
    ...
    * Upsampling is achieved through bilinear interpolation and can be replaced by transpose convolutional layers here.
    ...
    if filter_num_skip == 'auto':
        verbose = True
        filter_num_skip = [filter_num[0] for num in range(depth_-1)]
        
    if filter_num_aggregate == 'auto':
        verbose = True
        filter_num_aggregate = int(depth_*filter_num[0])
    ...
    X_decoder = unet_3plus_base(IN, filter_num, filter_num_skip, filter_num_aggregate, 
        ...
        for i in range(1, L_out):
            if not multi_input: 
                ...
                X = decode_layer(X, n_labels, pool_size, unpool, 
                 ...
                 X = Conv2D(n_labels, (1,1), padding='same', name='{}_output_conv_{}'.format(name, i-1))(X_decoder[i])
    ...
    if multi_input and deep_supervision:
        model = Model([IN,], OUT_stack)
