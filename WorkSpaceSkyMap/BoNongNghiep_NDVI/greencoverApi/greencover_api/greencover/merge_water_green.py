...
def combine_all(path_image, result_green, result_water):
    result_color = path_image.replace('.tif','_color.tif')
    if not os.path.exists(result_color):
        ...
        # mask_green[mask_green==0]=2
        # mask_green[mask_green==1]=0
        # mask_green[mask_green==2]=1
        # mask_nongreen = mask_green + msk
        # mask_nongreen[mask_nongreen==0]=4

        mask_water = result_water
        # if mask_water.all() != 0 :
        #     mask_water[mask_water==0]=2
        #     mask_water[mask_water==1]=0
        #     mask_water[mask_water==2]=1
            # mask_all = mask_nongreen + mask_water
        ...
        mask_water[msk==1]=0
        mask_all = mask_water
        # mask_all[mask_all==6]=2

        # mask_all[mask_all==5]=0
        # mask_all[mask_all==3]=1
        # mask_all[mask_all==4]=3
        combine_path = path_image.replace('.tif','_combine.tif')
        with rasterio.Env():
            profile = out_meta
            profile.update(
